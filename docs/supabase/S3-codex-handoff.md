...
> ⚠️ **사후 정정(2026-07-21)**: 본 문서가 cron을 **`0012_cron.sql`** 로 지시한 것은 **오류였다.**
> 원격(스테이징)에 이미 `0013_storage`가 적용돼 있어서, 0012는 "이미 적용된 마이그레이션 앞에 끼어드는" 파일이 되어
> `supabase db push`(배포)가 거부했다. **`0012_cron.sql` → `0015_cron.sql`으로 리넘버**해 해소함(로컬 최종 상태·pgTAP 197 동일).
> **교훈: 신규 마이그레이션 번호는 원격에 이미 배포된 최고 번호보다 뒤여야 한다.** 아래 본문의 `0012` 언급은 전부 `0015`로 읽을 것.
