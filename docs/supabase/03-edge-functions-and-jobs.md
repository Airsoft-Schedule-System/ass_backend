...
- 처리 순서:
  ...
  3. **타입 필터** — 앱 밖 도달이 필요한 것만 발송. 대상: `participation.decision` · `payment.requested` · `payment.decision` · `session.upcoming_reminder` · `session.changed`. **제외: `participation.confirmed`**(확인성 — 인앱으로 충분). 제외 대상이면 200 즉시 종료.
     - `participation.decision`(승인)과 `payment.requested`는 승인 시 동시 발생 → **가능하면 1통으로 병합**(중복 메일 방지·쿼터 절감). 구현: 같은 participation에 대해 직전 N초 내 발신 이력이 있으면 스킵하거나, 승인 경로에서 payment.requested만 메일 대상으로 삼는 방식 중 택1(구현 시 확정, 테스트로 고정).
